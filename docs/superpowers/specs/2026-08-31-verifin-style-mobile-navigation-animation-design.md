...
> 2026-09-08 / 1.5.2 修订：用户反馈动画卡顿、不连贯，当前实现保留主导航方向和单胶囊联动，改为单个活动路由场景。一级页面使用 24px / 220ms 的浏览器原生滑入，流程页面使用 10px / 180ms，胶囊使用 220ms transform 过渡；快速点击直接切换到最新目标。移除保留退出路由树的行为，动画参数在每次导航时固定。减少动态效果时关闭位移与淡入。下文关于 100% 整页横移、双场景退出和原有时长的描述保留为历史设计，不再代表当前实现。
...
- 原有贴边底栏改为左右各 12px 的悬浮白色圆角容器，底部位置继续叠加系统安全区；
- 容器使用轻边框、20px 圆角和克制阴影，不改变四个导航项的顺序；
- 导航容器内只存在一个真实选中胶囊；
- 胶囊宽度等于一个导航槽位减去内边距；
- 胶囊位置由当前一级页面索引计算；
